...
import { Eye, EyeOff, Stethoscope, ChevronDown } from 'lucide-react';
...
const SignupPage = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!specialization) { setErrorMsg('Please select your specialization'); return; }
    ...
    const { data, error } = await authService.signUp(email, password, fullName, specialization);
    ...
    if (error) {
      ...
      setSuccessMsg("Account created! Redirecting to profile setup...");
      setTimeout(() => navigate('/dashboard/profile-setup'), 1500);
    } else {
      setSuccessMsg("Check your email for the confirmation link, then complete your profile.");
  ...
  return (
    <div className="flex bg-[#EBF4FE] overflow-y-auto justify-center min-h-screen py-10 px-4 font-sans text-slate-800 relative w-full selection:bg-brand-blue selection:text-white md:items-center items-start">
      ...
      <div className="relative z-10 w-full max-w-[440px] mx-auto flex flex-col justify-start">

        {/* Logo */}
        <div className="flex flex-col items-center justify-center space-y-2 mb-8 select-none cursor-pointer">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold mb-2">Create Doctor Account</h1>
          <p className="text-slate-500 font-medium px-2 flex items-center justify-center gap-1.5">
        ...
        {/* Form */}
        <form onSubmit={handleSignup} className="flex flex-col w-full px-2">
          ...
          {/* Full Name */}
          <div className="mb-4">
            <label className="block text-slate-800 text-[15px] font-medium mb-2 pl-1">Full Name *</label>
            <input
              type="text"
          ...
          {/* Specialization */}
          <div className="mb-4">
            <label className="block text-slate-800 text-[15px] font-medium mb-2 pl-1">Specialization *</label>
          ...
          <div className="mb-6 relative">
            <label className="block text-slate-800 text-[15px] font-medium mb-2 pl-1">Confirm Password *</label>
            <div className="relative">
              <input
              ...
              >
                {showConfirmPassword ? <EyeOff size={18} /> : <Eye size={18} strokeWidth={2.2} />}
              </button>
            </div>
          </div>
          ...
          <p className="text-[11px] text-slate-400 text-center mb-4 leading-relaxed">
            By signing up, you agree to our <span className="text-blue-500 cursor-pointer hover:underline">Terms of Service</span> and confirm you are a licensed medical professional.
          </p>
        ...
        {/* Separator */}
        <div className="flex items-center justify-center mt-8 mb-6 select-none opacity-80 px-2">
        ...
        {/* Footer */}
        <div className="pb-6 text-center text-slate-700 font-medium text-[15px]">
          Already have an account? <Link to="/login" className="text-[#1A6BFF] hover:underline">Log in</Link>
        </div>
