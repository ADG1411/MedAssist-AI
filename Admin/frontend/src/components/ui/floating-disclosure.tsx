          ...
          <AnimatePresence mode="popLayout" initial={false}>
            {!isOpen ? (
              ...
              <motion.div key="open" className="flex shrink-0 flex-col gap-3">
                {items.map((item) => (
                  <motion.div
                    key={item.title}                    onClick={() => { if (item.onClick) item.onClick(); setIsOpen(false); }}                    className="flex flex-1 shrink-0 cursor-pointer items-center gap-2 rounded-xl p-1 hover:bg-[#f9f6f4] dark:hover:bg-white/5"
                    initial={{
                      opacity: 0,
                      filter: "blur(4px)",
                      y: 20,
