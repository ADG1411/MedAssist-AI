...
class ProfileScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final bloodGroup = user['bloodGroup']?.toString() ?? user['blood_group']?.toString() ?? '';
    final age = user['age']?.toString() ?? '';
    final gender = user['gender']?.toString() ?? '';
    final allergies = (user['allergies'] as List?)?.cast<String>() ?? [];
    final conditions = (user['chronicConditions'] as List?)?.cast<String>() ?? 
                       (user['chronic_conditions'] as List?)?.cast<String>() ?? [];
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 16),
            _buildInfoGrid(context, age, gender, bloodGroup),
            ...
            if (conditions.isNotEmpty || allergies.isNotEmpty) ...[
              ...
              if (conditions.isNotEmpty)
                _buildTagRow(context, 'Chronic', conditions, AppColors.danger),
  ...
  Widget _buildInfoGrid(BuildContext context, String age, String gender, String blood) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(color: Theme.of(context).cardColor, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppColors.border)),
      child: Row(
        children: [
          ...
          Container(width: 1, height: 40, color: AppColors.border),
          _infoTile('Blood', blood, Icons.water_drop),
        ],
      ),
    );
  }
